...
sub handler {
	...
	foreach my $item (@properties) {
		if($item->{'id'} =~ /^models$/) {
			...
			my @values;
			foreach my $value (qw(controller radio touch jivelite800x480 jivelite720x1280)) {
				my %v = (
					'value' => $value
				);
		...
		foreach my $item (@itemproperties) {
			if($item->{'id'} =~ /color$/) {
				...
				if($currentItem->{'period'} !~ /^d\d+/) {
					...
					push @values,{id=>'%l',name=>'Barometric Pressure (inHg)'};				
					push @values,{id=>'%p',name=>'Barometric Pressure (inHg)'};				
					push @values,{id=>'%L',name=>'Barometric Pressure (hPa)'};				
					push @values,{id=>'%P',name=>'Barometric Pressure (hPa)'};				
				...
				if($currentItem->{'period'} !~ /^d\d+/) {
					...
					if(defined($currentItem->{'period'}) && $currentItem->{'period'} eq "-1") {
						...
						push @values,{id=>'%wu_pressureIN',name=>'Barometric Pressure (inHg)'};				
						push @values,{id=>'%pressureIN',name=>'Barometric Pressure (inHg)'};				
						push @values,{id=>'%wu_pressureMB',name=>'Barometric Pressure (hPa)'};				
						push @values,{id=>'%pressureMB',name=>'Barometric Pressure (hPa)'};				
